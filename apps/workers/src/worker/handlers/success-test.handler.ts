 ...
 */
@Injectable()
export class SuccessTestHandler implements TaskHandler, OnModuleInit {
  // Task name constant - use this when submitting tasks via the API
  static readonly TASK_NAME = TaskType.SUCCESS_TEST;
   ...
   */
  onModuleInit(): void {
    this.taskRegistry.register(SuccessTestHandler.TASK_NAME, this);
    this.logger.info(
      'TASK_HANDLER',
      `Registered handler for task type: ${SuccessTestHandler.TASK_NAME}`,
      {},
    );
  }
